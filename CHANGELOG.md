...
- `TestCampaign` adapted for loading `PowerStep` files.

> [!WARNING]
> Order of arguments in `TestCampaign.from_filepaths` was changed.
...
- Is is now possible to re-open a `PowerStep` figure by clicking on interactive
  plot, even if a previous one was closed.

## [2.0.6] - 2026-06-05

- Right-click on interactive plot to change the plotted `Instrument`. Needs Qt.

## [2.0.5] - 2026-06-05

### Added

- In interactive plots, navigate between power steps with left/right arrows.
- In interactive plots, `PowerStep` display reduction info for each
  `Instrument`.
...
- Method `remove_post_treater`, defined for `Instrument`, `IMeasurementPoint`
  and `MultipactorTest`.
...
- `try`/`except` to avoid file saving error interrupting the run.

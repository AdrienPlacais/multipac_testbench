...
def instrument_excluder(
    ...
    if types_match(instruments_to_ignore, ABCMeta):
        raise InstrumentFilteringError(
            "name-based filters must be created with (list of) names or "
            "Instrument instances."
        )
    ...
    if types_match(instruments_to_ignore, Instrument):
        return instrument_excluder([str(i) for i in instruments_to_ignore])

    if types_match(instruments_to_ignore, str):

        as_set = set(instruments_to_ignore)

        def predicate(instrument_id: INSTRUMENT_ID) -> bool:
            """Reject instruments which name is in ``instrument_id``.
            ...
            match instrument_id:
                ...
                case str():
                    return instrument_id not in instruments_to_ignore

                case _:
                    raise InstrumentFilteringError(
                        f"instrument_id is a {type(instrument_id)}, which "
                        "is not supported."
                    )

        return predicate

    raise InstrumentFilteringError(f"{instruments_to_ignore = } is invalid.")
